...
const Appointment = () => {
  ...
  // Fetch doctor information from context
  const fetchDocInfo = () => {
    const docInfo = doctors.find((doc) => doc._id === docId);
    if (docInfo) {
      docInfo.slots_booked = docInfo.slots_booked || {}; // Ensure slots_booked is initialized
      setDocInfo(docInfo);
    } else {
  ...
  // Use effects to fetch data and handle updates
  useEffect(() => {
    fetchDocInfo();
  }, [doctors, docId]);
